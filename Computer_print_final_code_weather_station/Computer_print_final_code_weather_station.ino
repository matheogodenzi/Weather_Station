...
const float mq7_Vcc = 3.3; // To be confirmed
float mq7_R0 = 10000.0; // ohms To be calibrated

// MQ-131
const int mq131Pin = A9; // Pin 23 on teensy 4.0
const float mq131_RL = 10000.0; // supposed to be the correct one
const float mq131_Vcc = 3.3; //make sur it is the correct voltage to use
float R0 = 13000.0; // To be calibrated

// MPU6050 vars
float rollEst = 0, pitchEst = 0;
float rollAcc, pitchAcc;
float alpha = 0.80; // 80% percent given to the accelerometer, 20% to the gyro
...
void readMQ7() {
  ...
  float voltage = rawValue * (mq7_Vcc / 1023.0);

  // Calculate sensor resistance Rs
  float Rs = mq7_RL * ((mq7_Vcc - voltage) / voltage);

  // Calculate Rs/R0 ratio
  float ratio = Rs / mq7_R0;

  // Estimate CO ppm using log formula
  // You may need to adjust a and b based on your specific MQ-7 version and datasheet graph
  float a = -0.7; // TO BE UPDATED
  float b = 1.4;  // TO BE UPDATED
  float mq7_ppm = pow(10, (1/a * log10(ratio) - b/a));
...
void readMQ131() {
  ...
  float voltage = rawValue * (mq131_Vcc / 1023.0);
  
  // Sensor resistance
  float Rs = mq131_RL * ((mq131_Vcc - voltage) / voltage);
  ...
  float b = 0.90; // Approximated from datasheet
  float mq131_ppm = pow(10, (1/a * log10(ratio) - b/a));
